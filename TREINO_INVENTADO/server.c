...
int main(int argc, char *argv[]) {
    ...
    uint16_t recv_buffer[MAX_BUFFER];
    uint8_t send_buffer[MAX_BUFFER];
    ...
    while(1){
        ...
        if ((udp_read_bytes = recvfrom(udp_server_socket, recv_buffer, sizeof(recv_buffer), 0, (struct sockaddr *) &udp_client_endpoint, &udp_client_endpoint_length)) == -1)
            ERROR(34, "Can't recvfrom client");
        printf("ok.  (%d bytes received)\n", (int)udp_read_bytes);


        int total_numbers=udp_read_bytes/sizeof(recv_buffer[0]);
        ...
        if ((udp_sent_bytes = sendto(udp_server_socket,send_buffer, sizeof(uint8_t)*total_numbers, 0, (struct sockaddr *) &udp_client_endpoint, udp_client_endpoint_length)) == -1)
