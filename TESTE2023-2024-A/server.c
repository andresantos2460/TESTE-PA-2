...
int main(int argc, char *argv[]) {
    ...
    if ((udp_read_bytes = recvfrom(udp_server_socket,recv_buffer, sizeof(recv_buffer)-1, 0, (struct sockaddr *) &udp_client_endpoint, &udp_client_endpoint_length)) == -1)
    ...
    for (int i=0; i<udp_read_bytes;i++) {
        ...
        while (!flag) {
            int bitAleatorio=rand()%8;

            novaLetra=recv_buffer[i];
            novaLetra^=(1<<bitAleatorio);

            if((unsigned char)novaLetra>=33 &&(unsigned char)novaLetra<=127){
                flag=1;
            }
        }


        send_buffer[i]=novaLetra;

    }

    send_buffer[udp_read_bytes]='\0';
    printf("transfored :'%s'\n",send_buffer);
    ...
    if ((udp_sent_bytes = sendto(udp_server_socket,send_buffer,strlen(send_buffer), 0, (struct sockaddr *) &udp_client_endpoint, udp_client_endpoint_length)) == -1)
