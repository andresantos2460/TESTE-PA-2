...
int main(int argc, char *argv[]) {
	...
	pedido.n_estudantes=htonl(args.student_arg);
	pedido.telefone=htonl(args.phone_arg);
	pedido.operacao=args.operation_arg;
...
//--- UDP Client :: ligação ------  ( ** não copiar este comentário ** )
	...
	uint32_t codigo_recebido;
    

    //============= CODIGO ANTES DE ENVIAR ==================//



	// UDP IPv4: "sendto" para o servidor
	printf("a enviar dados para o servidor... "); fflush(stdout);
	if ((udp_sent_bytes = sendto(udp_client_socket, &pedido, sizeof(pedido), 0, (struct sockaddr *) &udp_server_endpoint, udp_server_endpoint_length)) == -1)
...
// UDP IPv4: "recvfrom" do servidor (bloqueante)
	...
	if ((udp_read_bytes = recvfrom(udp_client_socket, &codigo_recebido, sizeof(codigo_recebido), 0, (struct sockaddr *) &udp_server_endpoint, &udp_server_endpoint_length)) == -1)
		ERROR(25, "Can't recvfrom server");
	printf("ok.  (%d bytes received)\n", (int)udp_read_bytes);	
	
	codigo_recebido=ntohl(codigo_recebido);
