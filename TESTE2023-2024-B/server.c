...
//--- UDP Server :: ligação  ------ ( ** não copiar este comentário ** )
    ...
    while (1) {
        ...
        int num_elementos = udp_read_bytes / sizeof(buffer[0]);
        ...
        if ((udp_sent_bytes = sendto(udp_server_socket, vetor_contador, sizeof(uint8_t)*num_elementos, 0, (struct sockaddr *) &udp_client_endpoint, udp_client_endpoint_length)) == -1)
