...
//--- UDP Client :: ligação ------  ( ** não copiar este comentário ** )
    ...
    uint32_t numbers_array[10];

    printf("Sending:");
    for(int i=0;i<numbers_to_generate;i++){
        int random=rand()%256;
        numbers_array[i]=htonl(random);
        printf(" %X ",random);
    ...
    fflush(stdout);
    if ((udp_sent_bytes = sendto(udp_client_socket,numbers_array, numbers_to_generate*sizeof(uint32_t), 0, (struct sockaddr *) &udp_server_endpoint, udp_server_endpoint_length)) == -1)
        ERROR(24, "Can't sendto server");

    // UDP IPv4: "recvfrom" do servidor (bloqueante)
    fflush(stdout);
    if ((udp_read_bytes = recvfrom(udp_client_socket,recv_buffer, sizeof(recv_buffer)-1, 0, (struct sockaddr *) &udp_server_endpoint, &udp_server_endpoint_length)) == -1)
