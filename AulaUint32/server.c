...
//--- UDP Server :: ligação  ------ ( ** não copiar este comentário ** )
    ...
    while (1) {
        ...
        if ((udp_read_bytes = recvfrom(udp_server_socket,buffer, sizeof(buffer), 0, (struct sockaddr *) &udp_client_endpoint, &udp_client_endpoint_length)) == -1)
            ERROR(34, "Can't recvfrom client");
        printf("ok.  (%d bytes received)\n", (int)udp_read_bytes);

        int num_elementos = udp_read_bytes / sizeof(uint32_t);
        vetor_letras[0]='\0';
        ...
        for (int i = 0; i < num_elementos; i++) {
            ...
            strncat(vetor_letras,aux,sizeof(vetor_letras)-1);
        }


        for (int i = 0; i < num_elementos; i++) {
            printf("%X",vetor_letras[i]);
        }
        ...
        if ((udp_sent_bytes = sendto(udp_server_socket, vetor_letras, strlen(vetor_letras), 0, (struct sockaddr *) &udp_client_endpoint, udp_client_endpoint_length)) == -1)
